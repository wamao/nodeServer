...
createTable(chosen) // 精选商品列表

createTable(discount) // 折扣
...
// 商品查询（关键字查询）
let  searchGoods=(keyword,page)=>{
  //let  _sql=`select * from goods where goodsName like "${value}";`
    let  _sql=`select * from goods where goodsName like '%${keyword}%' limit ${(page-1)*10},10;`
    return query(_sql,[]);
}
...
//添加商品精选
let addChosen=(value)=>{
  let _sql = "insert into chosen set goodsId=?;"
  return query( _sql, value ); 
}
...
let  chosenList=()=>{
 // _sql=`select * from usercoupon left join coupon on usercoupon.couponId=coupon.couponId where usercoupon.userId='${value}';`
  let _sql="select * from chosen left join goods on chosen.goodsId=goods.goodsId"
  return query( _sql);  
}


//添加折扣精选
let addDiscount=(value)=>{
  let _sql = "insert into discount set goodsId=?,discount=?;"
  return query( _sql, value ); 
}
...
let  discountList=()=>{
  // _sql=`select * from usercoupon left join coupon on usercoupon.couponId=coupon.couponId where usercoupon.userId='${value}';`
   let _sql="select * from discount left join goods on discount.goodsId=goods.goodsId"
   return query( _sql,[]);  
 }
